...
int get_sem(){
	...
	if((int)key == -1){
		sprintf(mensaje, "Error [ftok]: el path no existe o no puede ser accedido por el proceso.\n");
	...
	if(semId == -1){
		sprintf(mensaje, "Error [semget]: al obtener el identificador del semáforo.\n");
		write (STDOUT_FILENO, mensaje, strlen(mensaje));
		exit(0);
	}
...
int get_sem_value (int semId, int semnum){
	...
	if (i == -1) {
		sprintf(mensaje, "Error [semctl]: al obtener el valor del semáforo.\n");
		write (STDOUT_FILENO, mensaje, strlen(mensaje));
		exit(0);
	}
	return i;
...
void set_sem_value (int semid, int semnum, int value){	
	...
	if (i == -1) {
		sprintf(mensaje, "Error [semctl]: al actualizar el valor del semáforo.\n");
		write (STDOUT_FILENO, mensaje, strlen(mensaje));
		exit(0);
	}
}
...
void sem_op(int semId, int semnum, int op){
	...
	if (semop(semId, op_buf, 1) == -1) {
		sprintf(mensaje, "Error [semop]: al modificar el semáforo %d [pid=%d].\n", semnum, getpid());
		write (STDOUT_FILENO, mensaje, strlen(mensaje));
		exit(0);
	}
}
...
void del_sem(int semId){
	...
	if(err == -1){
		sprintf(mensaje, "Error [semctl]: al eliminar el semáforo.\n");
		write (STDOUT_FILENO, mensaje, strlen(mensaje));
		exit(0);	
	}
}
